/*!
 * GSAP SVG Plug-in
 * Version 0.1 (i.e., totally experimental and probably broken somehow)
 *
...
(window._gsQueue || (window._gsQueue = [])).push( function() {
  ...
  var _NaNExp = /[^\d\-\.]/g,
    ...
    SvgPlugin = window._gsDefine.plugin({
      ...
      init: function(domObj, value, tween) {

        // Here we provide a simple wrapper for the SVG DOM object,
        // allowing use to manipulate its values in a jQuery-like fashion.
        var target = {
          attr: function (properties, value) {
            if (!properties) return false;
            if (properties.constructor === String) {
              domObj.setAttributeNS(null, properties, value);
            } else if (properties instanceof Object) {
                for (var key in properties) {
                    if (properties.hasOwnProperty(key)) {
                      domObj.setAttributeNS(null, key, properties[key]);
                    }
                }
            }
            return this;
